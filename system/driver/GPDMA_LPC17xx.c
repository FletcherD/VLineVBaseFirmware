...
#include "../driver/GPDMA_LPC17xx.h"
...
#define GPDMACH0          ((GPDMA_CHANNEL_REG   *) LPC_GPDMACH0_BASE )
...
int32_t GPDMA_Initialize (void) {
  ...
  LPC17xx_GPDMA->DMACIntTCClear = 0xFFU;
  LPC17xx_GPDMA->DMACIntErrClr  = 0xFFU;
...
#if defined (LPC175x_6x)
  if ((peri > 15U) || (peri <  8U) || (sel > 1U)) { return -1; }

  // Configure DMA Request Select for GPDMA input 8 - 15
  LPC_SC->DMAREQSEL = (sel) ? (LPC_SC->DMAREQSEL |  (1<<(peri - 8))) :\
                              (LPC_SC->DMAREQSEL & ~(1<<(peri - 8)));
#elif defined (LPC177x_8x)
  if ((peri > 15U) ||(sel > 1U)) { return -1; }

  // Configure DMA Request Select for GPDMA input 0 - 15
  LPC_SC->DMAREQSEL = (sel) ? (LPC_SC->DMAREQSEL |  (1<<peri)) :\
                              (LPC_SC->DMAREQSEL & ~(1<<peri));
#endif
  return 0;
}
...
int32_t GPDMA_ChannelConfigure (uint8_t              ch,
  ...
  LPC17xx_GPDMA->DMACIntTCClear = (1U << ch);
  ...
  LPC17xx_GPDMA->DMACConfig = GPDMA_CONFIG_E;
  while ((LPC17xx_GPDMA->DMACConfig & GPDMA_CONFIG_E) == 0U);
...
void DMA_IRQHandler (void) {
  ...
  for (ch = 0U; ch < GPDMA_NUMBER_OF_CHANNELS; ch++) {
    if (LPC17xx_GPDMA->DMACIntStat & (1U << ch)) {
      ...
      if (LPC17xx_GPDMA->DMACIntTCStat & (1U << ch)) {
        ...
        // DMA error interrupt
        if (LPC17xx_GPDMA->DMACIntErrStat & (1U << ch)) {
          dma_ch->CONFIG  = 0U;
          dma_ch->CONTROL = 0U;
